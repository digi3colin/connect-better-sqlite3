...
module.exports = function(connect) {
  ...
  class SQLiteStore{
    ...
    constructor(options = {}){
      ...
      this.dbPath   = options.dir || '.';
      this.filename = options.filename || (this.table + '.sqlite');
    ...
    getConnection(request){
      const host = (request)? request.headers.host.split(':')[0] : 'default';
      let path = (this.dbPath + '/' + this.filename).replace(/\{host\}/g, host);
    ...
    set(sid, sess, fn, opts={}){
      try {
        ...
        db.prepare('INSERT OR REPLACE INTO ' + this.table + ' (sid, expired, sess) VALUES (?, ?, ?)').run(sid, expired, sess);
